<template>
	<view>
		...
		<view class="index-tips" v-if="isParty==0">未查询到您的缴费单信息，请与收费单位核实。</view>
		<block v-else>
			<view class="index-content-top">
				...
				<view class="index-content-top-right" @click="qieStatus">
					<image src="/static/images/party_img2.png"></image>
					<text>历史缴费</text>
				</view>
			</view>
			<view class="index-content-mes">
				<block v-for="(item,index) in list" :key="index">
					<view class="index-content-mes-li" @click="goPay" :data-id="item.id" :data-month="item.date" :data-money="item.party_money">
						<view class="index-content-mes-li-top">
							<text class="index-content-mes-li-top-text1"></text>
							<text class="index-content-mes-li-top-text2">{{item.date}}份{{price_title}}</text>
							<text class="index-content-mes-li-top-text3">{{item.party_money}}元</text>
						</view>
						<view class="index-content-mes-li-top-bottom">等待缴费</view>
					</view>
				</block>
			</view>
		</block>
...
<script>
	...
	export default {
			...
			if(userInfo.id){
				...
				request.requestApi(url,{uid:userInfo.id,token:userInfo.token}).then(res=>{
					if(res.data.code!=1001){
						_this.isParty = 1;
						_this.list = res.data.info[0].info;
						_this.price_title = res.data.info[0].dang;	
					}
		...
		methods: {
			goPay(e){ //去缴费
				uni.navigateTo({
					url:'/pages/party_pay_2/index?yearmonth_id=' + e.currentTarget.dataset.id
				})
			},
			/*切换成缴费明细*/
			qieStatus(){
				uni.navigateTo({
					url:'/pages/party_pay_1/index'
				})
			},
		}
	}
